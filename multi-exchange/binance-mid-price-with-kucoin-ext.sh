...
INSERT INTO "binance-mid-price-with-kucoin-ext"
WITH base AS (
  SELECT
    __time,
    CASE
...
kucoin AS (
  SELECT
    ...
    extractionTimeMicro AS kucoinExtractionTimeMicro,
...
SELECT
  ...
  b.extractionTimeMicro AS binanceExtractionTimeMicro,
  b."value" AS binance_value,
  k.kucoinSequence,
  k.kucoinExtractionTimeMicro,
  k.kucoin_value
FROM annotated b
LEFT JOIN kucoin k
